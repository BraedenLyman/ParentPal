...
export default function PhotoGallery() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const accountType = userResponse.data.user.account_type;
                setAccountId(userId);
                setUserType(accountType);

                if (accountType === 'babysitter' || isBabysitter) {
                    // For babysitters: fetch children they have access to
                    const childrenResponse = await axios.get(
                        `${API_URL}/api/babysitter-sharing/children/${userId}`,
                        { withCredentials: true }
                    );
                    setBabies(childrenResponse.data.children || []);

                    if (childrenResponse.data.children && childrenResponse.data.children.length > 0) {
                        setSelectedBabyId(childrenResponse.data.children[0].baby_id);
                    }

                    // Fetch photos for babies accessible to babysitter
                    const photosResponse = await axios.get(
                        `${API_URL}/api/photo-gallery/babysitter/${userId}`,
                        { withCredentials: true }
                    );
                    setPhotos(photosResponse.data);
                } else {
                    // For parents: fetch their own babies
    ...
    const handleUpload = async () => {
        ...
        try {
            // For babysitters, we need to find the parent_id of the selected baby
            let parentIdToUse = accountId;

            if (userType === 'babysitter' || isBabysitter) {
                const selectedBaby = babies.find(baby => baby.baby_id === parseInt(selectedBabyId));
                if (selectedBaby && selectedBaby.parent_id) {
                    parentIdToUse = selectedBaby.parent_id;
                }
            }
            ...
            formData.append('parent_id', parentIdToUse);
            ...
            // Refresh photos based on user type
            const endpoint = (userType === 'babysitter' || isBabysitter)
                ? `/api/photo-gallery/babysitter/${accountId}`
                : `/api/photo-gallery/parent/${accountId}`;
