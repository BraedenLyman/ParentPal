...
router.put('/:growth_id', validateGrowthData, async (req, res) => {
    const { growth_id } = req.params;
    const { baby_id, weight, height, date } = req.body;
    ...
    try {
        const result = await pool.query(
            ...
            [baby_id, weight, height, date, growth_id]
        );
...
router.delete('/:growth_id', async (req, res) => {
    const { growth_id } = req.params;
    ...
    try {
        ...
        res.json({ message: 'Growth record deleted successfully', growth_id: result.rows[0].growth_id });
    } catch (err) {
